...
var _ = BeforeSuite(func() {
	...
	fmt.Println("Upstream Test Start")
...
var _ = Describe("Integration", func() {
	Context("Downstream Service API Test", func() {
		It("Should Success when no request body is provided", func() {
			request := &TestRequest{}
			statusCode, response, err := CallAPI("GET", host, "withUpstreamAPI", request)
		...
		It("Should Fail when given trigger is passed 2", func() {
			request := &TestRequest{
				Expectation: "upstream_error",
			}
			statusCode, response, err := CallAPI("GET", host, "withUpstreamAPI", request)
			Expect(err).To(BeNil(), "call downstream API should success")
			Expect(statusCode).To(Equal(400), "status code should be 200.")
			Expect(response.Code).To(Equal(4), "service code should be 0")
		})
	})
})
